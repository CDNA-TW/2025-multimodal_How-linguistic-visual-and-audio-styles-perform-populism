"""
Scrapes all comments for each already-identified post (keycode) in a
filtered video dataset (e.g. dataset/ig_video_visualValid_N584_0901-1104_zscoreEngagement_0328.csv),
appending one row per comment (party/state/name/keycode/post_date/
commenter_username/comment_text/comment_date/comment_likes) to an output
CSV. Resumable: posts whose keycode is already in the output CSV are
skipped.
...
in the source file (plus paginated requests per comment thread), so it is
even more rate-limit-sensitive.
...
- OUTPUT_CSV: where to append scraped comments.
"""
import os
import time
import emoji
...
save_file = os.environ.get("OUTPUT_CSV", "./output/ig_video_comments.csv")
    ...
    try:
        ...
        for x in post.get_comments():
            comments_data.append({
                ...
                "comment_text_demojize": emoji.demojize(x.text) if x.text else "",
                "comment_date": x.created_at_utc.strftime('%Y-%m-%d %H:%M:%S'),
                "comment_likes": x.likes_count
            })

        if not comments_data:
            print(f"No comments found for post {shortcode}")
            return
        ...
        print(f"Done scraping {shortcode}, saved to {save_file}.")
...
for _, row in tqdm(df_filtered.iterrows(), total=len(df_filtered)):
    ...
    state = row["state"]
    name = row["name"]
    scrape_data(code, party, state, name)
